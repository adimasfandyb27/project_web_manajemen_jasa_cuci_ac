...
@section('content')
    ...
    <div
        ...
        <div class="relative z-10">
            ...
            <p class="text-emerald-50 max-w-2xl">
                Lengkapi data unit AC yang Anda miliki agar informasi service lebih akurat.
            </p>
        </div>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('customer.ac-units.store') }}" method="POST"
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 text-sm">
                    Isi data spesifikasi unit AC Anda dengan benar.
                </p>
            </div>
                ...
                <div>
                    ...
                    <select name="ac_brand_id" required
                        ...
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}" {{ old('ac_brand_id') == $brand->id ? 'selected' : '' }}>
                                {{ $brand->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="ac_type_id" required
                        ...
                        @foreach ($types as $type)
                            <option value="{{ $type->id }}" {{ old('ac_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="ac_capacity_id" required
                        ...
                        @foreach ($capacities as $capacity)
                            <option value="{{ $capacity->id }}" {{ old('ac_capacity_id') == $capacity->id ? 'selected' : '' }}>
                                {{ $capacity->label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-6">
                ...
                <textarea name="catatan" rows="3"
                    ...
                    class="w-full border-gray-200 rounded-2xl focus:ring-emerald-500 focus:border-emerald-500">{{ old('catatan') }}</textarea>
            </div>
